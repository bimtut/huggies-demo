...
function accordion() {
  const accordionContainer = document.getElementsByClassName("accordion-faq");
  for (let index = 0; index < accordionContainer.length; index++) {
    ...
    addClass(
      button,
      "absolute block right-0 transform -translaye-y-1/2 focus:outline-none transition duration-200 rotate-0 md:pr-4"
    );
...
if(window.innerWidth > 100) window.addEventListener("load", accordion)
